...
export default async function SheetPrintPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="print-page min-h-screen overflow-x-auto bg-[#e7e2d4] py-8 print:bg-white print:py-0">
      <PrintToolbar
        sheetId={sheet.id}
        title={sheet.title}
        altHref={`/sheets/${sheet.id}/print/gabarito`}
        altLabel="Ver gabarito"
      />
      <div className="mx-auto w-fit">
        <SheetDocument title={sheet.title} pageSettings={pageSettings} coverLayout={coverLayout} items={items} mode="print" />
      </div>
    </div>
  );
}
